...
internal class AtomicFlowRecoveryService(
    IServiceScopeFactory scopeFactory,
    IAtomicFlowRollbackRegistry registry,
    ILogger<AtomicFlowRecoveryService> logger
) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        await using var scope = scopeFactory.CreateAsyncScope();
        var repository = scope.ServiceProvider.GetRequiredService<IAtomicFlowRepository>();
        ...
        logger.LogWarning(
            "AtomicFlow recovery: found {Count} pending rollback(s) from a previous run",
            pending.Count
        );
